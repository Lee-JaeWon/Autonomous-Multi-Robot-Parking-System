...
DWAPlanner::DWAPlanner(void)
    : local_nh("~"), local_goal_subscribed(false), scan_updated(false), local_map_updated(false), odom_updated(false) , global_path_update(true), vel_stop_flag(true)
{
    ...
    if (ros::param::get("~namespace", s))
        ROS_INFO("dwa node got param: %s", s.c_str());
    else
        ROS_ERROR("dwa Failed to get param '%s'", s.c_str());
    ...
    {
        scan_sub = nh.subscribe(ns + "/scan", 1, &DWAPlanner::scan_callback, this);
    }
    else
    {
...
// void DWAPlanner::path_callback(const nav_msgs::PathConstPtr& msg)
// {
//     int i=0;
//     if((local_goal.pose.position.x - current_position.pose.position.x) + (local_goal.pose.position.y - current_position.pose.position.y) <0.1){
//     i++;
//     }
//     local_goal = msg->poses[i].pose.position;
// }
...
void DWAPlanner::global_path_callback(const nav_msgs::PathConstPtr &msg)
{
    global_path = *msg;
    ROS_INFO("%ld",global_path.poses.size());
    if (global_path.poses.size() < 5)
        global_path_update = false;
    else 
        global_path_update = true;
}

void DWAPlanner::cmd_velstop_Callback(const std_msgs::Bool::ConstPtr &msg)
{
    vel_stop_flag = msg->data;
}
    ...
    while (ros::ok())
    {
        ...
        if (input_updated && local_goal_subscribed && odom_updated)
        {
            ...
            if (goal.segment(0, 2).norm() > GOAL_THRESHOLD)
            {
                ...
                visualize_trajectory(best_traj, 1, 0, 0, selected_trajectory_pub, path_pub);
            }
            else
            {
                cmd_vel.linear.x = 0.0;

                ROS_INFO("norm < 0.3 lv =0.0");
...
void DWAPlanner::visualize_trajectory(const std::vector<State> &trajectory, const double r, const double g, const double b, const ros::Publisher &pub, const ros::Publisher &pub2)
{
    visualization_msgs::Marker v_trajectory;
    nav_msgs::Path path;
    path.header.frame_id = s + '/' + "base_link";
    v_trajectory.header.frame_id = ROBOT_FRAME;
    v_trajectory.header.stamp = ros::Time::now();
    v_trajectory.color.r = r;
    v_trajectory.color.g = g;
    ...
    v_trajectory.pose = pose;
    geometry_msgs::Point p;
    std::vector<geometry_msgs::PoseStamped> path_list;
    for (const auto &pose : trajectory)
    {
        geometry_msgs::PoseStamped path_term;
        p.x = pose.x;
        p.y = pose.y;
        v_trajectory.points.push_back(p);
        path_term.pose.position.x = pose.x;
        path_term.pose.position.y = pose.y;
        path_list.push_back(path_term);
    }
    path.poses = path_list;
    pub.publish(v_trajectory);
    if (global_path_update)
    {
        ROS_INFO("local path pub");
        pub2.publish(path);
    }
}
